...
class JsonResponse extends Response
{
    ...
    public function setData($data = array())
    {
        ...
        try {
			...
			//проверка на недопустимый символ в имени файла,
			if (isset($data->files))
				{
					foreach ($data->files as $k=>$n)
						{
							$data->files[$k]["name"]=mb_convert_encoding($data->files[$k]["name"],"UTF-8","UTF-8");
						}
				}
...
//$data=mb_convert_encoding($data,"UTF-8","UTF-8");
        ...
        if (JSON_ERROR_NONE !== json_last_error()) {echo '<pre>';print_r($data);
